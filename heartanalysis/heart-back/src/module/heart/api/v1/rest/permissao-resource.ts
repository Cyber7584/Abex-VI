...
import { PermissaoPermissaoEnum } from "@heart/enum/permissao-enum";
...
routeBuilder.get(
	router,
	process.env.BASE_URL + "/api/v1/rest/permissao",
	[
		securityHelper.verifyJWT,
		permissaoService.validarPermissoes(PermissaoPermissaoEnum.VISUALIZAR),
	],
	async function (req: Request, resp: Response) {
		...
		if (!req.headers.is_paginated) {
			data = await model.Permissao.findAll({
				// include: ['categoria_permissao'],
				order: [["id", "asc"]],
			});
		} else {
			data = await paginate({
...
routeBuilder.get(
	router,
	process.env.BASE_URL + "/api/v1/rest/permissao/:id",
	[
		securityHelper.verifyJWT,
		permissaoService.validarPermissoes(PermissaoPermissaoEnum.VISUALIZAR),
	],
	async function (req: Request, resp: Response) {
		...
		const item = await model.Permissao.findByPk(id, {
			include: ["categoria_permissao"],
		});
...
routeBuilder.post(
	...
	[
		securityHelper.verifyJWT,
		permissaoService.validarPermissoes(PermissaoPermissaoEnum.CRIAR),
	],
	async function (req: Request, resp: Response) {
		await model.sequelize.transaction(async (transaction: Transaction) => {
			let item = await model.Permissao.create(req.body);
...
routeBuilder.delete(
	...
	[
		securityHelper.verifyJWT,
		permissaoService.validarPermissoes(PermissaoPermissaoEnum.EXCLUIR),
	],
	async function (req: Request, resp: Response) {
		const id = req.params.id;
		const item = await model.Permissao.findByPk(id, {
